...
$pdf->Cell(400,16,utf8_decode("REPORTE  DE STOCK DE ROLLOS AL  ".date("d-m-Y H:i") ),0,0,'C');
$pdf->ln(15);
...
$pdf->Cell(400, 12, "TELARES", 1, 0, 'C',1);
$pdf->ln(13);
 $art_unico_tel = $planificacion->artUnicoStock($telares);
if($art_unico_tel){ 
	...
	$pdf->ln();
		$pdf->SetFont('Arial','',10);
		 $detalle_resumen_stock_tel_op =$planificacion->ordenPedCan($telares,$list_codart['codart']);
			if($detalle_resumen_stock_tel_op){ 
				foreach($detalle_resumen_stock_tel_op as $list_ped){
					if($list_codart['codart']== $list_ped['codart']){
					 ...
					 $cadena='';
					 $art_det_kanban_tel = $planificacion->DetstockXkanbanXproceso($telares,$list_ped['codart'],$list_ped['prokandet_nroped']);
...
$pdf->ln(13);
 $art_unico_lam = $planificacion->artUnicoStock($laminado);
if($art_unico_lam){ 
	...
	$pdf->ln();
		$pdf->SetFont('Arial','',10);
		 $detalle_resumen_stock_lam_op =$planificacion->ordenPedCan($laminado,$list_codart['codart']);
			if($detalle_resumen_stock_lam_op){ 
				foreach($detalle_resumen_stock_lam_op as $list_ped){
					...
					//  $pdf->Cell(36,10,$list_ped['prokandet_nroped'],1,0,'C');
					 $cadena='';
					 $art_det_kanban_lam = $planificacion->DetstockXkanbanXproceso($laminado,$list_ped['codart'],$list_ped['prokandet_nroped']);
...
$pdf->SetFillColor(2,157,116);
$pdf->Cell(400, 12, utf8_decode("IMPRESIÓN"), 1, 0, 'C',1);
$pdf->ln(13);
 $art_unico_imp = $planificacion->artUnicoStock($impresion);
if($art_unico_imp){ 
	...
	$pdf->ln();
		$pdf->SetFont('Arial','',10);
		 $detalle_resumen_stock_imp_op =$planificacion->ordenPedCan($impresion,$list_codart['codart']);
			...
			if($detalle_resumen_stock_imp_op){ 
				foreach($detalle_resumen_stock_imp_op as $list_ped){
					...
					//  $pdf->Cell(36,10,$list_ped['prokandet_nroped'],1,0,'C');
					 $cadena='';
					 $art_det_kanban_imp = $planificacion->DetstockXkanbanXproceso($impresion,$list_ped['codart'],$list_ped['prokandet_nroped']);
